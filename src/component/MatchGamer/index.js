...
import GameImageItem from '../GameImageItem'
...
class MatchGamer extends Component {
  state = {
    timer: 60,
    scores: 0,
    randomImageUrl: imagesList[0].imageUrl,
    randomImageId: imagesList[0].id,
    activeImageStatus: tabsList[0].tabId,
    gameStatus: true,
  }
  ...
  setTimer = () => {
    ...
    // const getRandomImageIndex = Math.floor(Math.random() * imagesList.length)
    // const randomImage = imagesList[getRandomImageIndex]
  ...
  onTabClicked = tabId => {
    this.setState({activeImageStatus: tabId})
  }

  checkImageIsMatched = id => {
    const {randomImageId, scores, timer} = this.state
    const isMatched = id === randomImageId
    if (isMatched) {
      this.setState(prevState => ({scores: prevState.scores + 1}))
  ...
  playAgainClicked = () => {
    this.setState({scores: 0, timer: 60, gameStatus: true})
    this.generateRandomImage()
  }
  ...
  render() {
    ...
    const filteredImageList = imagesList.filter(eachImage =>
      eachImage.category
        .toLowerCase()
        .includes(activeImageStatus.toLowerCase()),
    )
    return (
      <>
        <div className="app-bg-container">
          <Header time={timer} score={scores} />
          <div className="match-match-container">
            {gameStatus ? (
              <>
                <img
                  src={randomImageUrl}
                ...
                />
                <ul className="tab-list-items-container">
                  {tabsList.map(eachTab => (
                    <TabsList
                      key={eachTab.tabId}
                      eachTabDetails={eachTab}
                      onTabClicked={this.onTabClicked}
                      activeButton={eachTab.tabId === activeImageStatus}
                    />
                  ))}
                </ul>
                <ul className="images-list-container">
                  {filteredImageList.map(eachImageDetails => (
                    <GameImageItem
                      key={eachImageDetails.id}
                      imageDetails={eachImageDetails}
                      checkImageIsMatched={this.checkImageIsMatched}
                    />
                  ))}
                </ul>
              </>
              ...
              <div className="failure-image-container">
                <img
                  ...
                  className="trophy"
                />
                <p className="yours-score-heading">YOUR SCORE</p>
